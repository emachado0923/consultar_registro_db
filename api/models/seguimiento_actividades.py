...
class UltimaActualizacionResponse(BaseModel):
    """Ver GET /seguimiento/ultima-actualizacion — a pedido de Migue, para
    la tarjeta de "fecha de actualización" de cada módulo en el selector de
    apps. `fecha` = MAX(ultima_actualizacion) de actividades_convenio_seg_mc
    (columna ya existente, se actualiza cada vez que se guarda avance/
    estado/comentario de una actividad) — None solo si no hay ninguna
    actividad registrada todavía."""
