...
@router.post("/debug-422")
async def debug_422(request: dict):
    """Endpoint para diagnosticar el error 422"""
    try:
        # Intentar parsear como ReintegrosCreate para ver dónde falla
        reintegro = ReintegrosCreate(**request)
        return {
            "status": "success", 
            "message": "Datos válidos",
            "data": reintegro.model_dump()
        }
    except Exception as e:
        return {
            "status": "error",
...
def get_schema_info():
    """Obtener información del schema actual"""
    schema = ReintegrosCreate.model_json_schema()
    return {
...
@router.post("/echo")
async def echo_data(reintegro: ReintegrosCreate):
    """Simplemente devuelve los datos recibidos"""
    return {
        "status": "success",
        "received_data": reintegro.model_dump(),
        "data_types": {k: type(v).__name__ for k, v in reintegro.model_dump().items()}
    }