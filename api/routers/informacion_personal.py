...
def create_informacion_personal(item: InformacionPersonal, session: SessionDep) -> InformacionPersonal:
    session.add(item)
    session.commit()
...
def update_informacion_personal(
    docconvfondo: str, data: InformacionPersonalUpdate, session: SessionDep
) -> InformacionPersonal:
...
def delete_informacion_personal(docconvfondo: str, session: SessionDep):
    item = session.get(InformacionPersonal, docconvfondo)
    if not item:
        raise HTTPException(status_code=404, detail="Registro no encontrado")
    session.delete(item)
